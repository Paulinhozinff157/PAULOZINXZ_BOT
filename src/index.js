 ...
 * Substitua a imagem "paulozinxz-bot.png" por outra de sua preferência!
 * Só não esqueça de manter o nome "paulozinxz-bot.png"
 *
 ...
 * ---------------- 🚀 IMPORTANTE 🚀 ----------------
 *
 * Leia o tutorial completo em: https://github.com/guiireal/paulozinxz-bot?tab=readme-ov-file#instala%C3%A7%C3%A3o-no-termux-
 *
...
import { readCommandImports } from "./utils/index.js";
...
function cleanTemporaryCache() {
  ...
  for (const entry of fs.readdirSync(TEMP_DIR, { withFileTypes: true })) {
    ...
    // Nunca remove o log aberto pelo processo nem arquivos recentes.
    if (entry.name === "wa-logs.txt") continue;

    try {
      const stats = fs.statSync(entryPath);
      if (now - stats.mtimeMs < maxAge) continue;
      ...
      errorLog(`Não foi possível limpar o cache ${entry.name}: ${error.message}`);
    }
  }
}

cleanTemporaryCache();
setInterval(cleanTemporaryCache, 60 * 60 * 1000);
