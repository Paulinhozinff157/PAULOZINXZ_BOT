  ...
  for (const webMessage of messages) {
    ...
    try {
      ...
      if (
        ...
        isActiveAutoRevealGroup(webMessage.key.remoteJid)
      ) {
        const viewOnceMediaType = getViewOnceMediaType(webMessage);
        if (viewOnceMediaType) {
          const wasRevealed = await revealViewOnceMedia({
            webMessage,
            mediaType: viewOnceMediaType,
            ...commonFunctions,
          });
          if (wasRevealed) {
            continue;
