...
async function getConfiguredChannel(socket) {
  if (cachedChannelMetadata) return cachedChannelMetadata;
  if (channelMetadataPromise) return channelMetadataPromise;

  const inviteCode = CHANNEL_INVITE_URL.match(
    /whatsapp\.com\/channel\/([A-Za-z0-9_-]+)/i,
  )?.[1];

  if (!inviteCode || typeof socket.newsletterMetadata !== "function") {
    return null;
  }

  channelMetadataPromise = socket
    .newsletterMetadata("invite", inviteCode)
    .then((metadata) => {
      cachedChannelMetadata = metadata?.id ? metadata : null;
...
export function loadCommonFunctions({ socket, webMessage }) {
  ...
  const sendChannelCard = async () => {
    try {
      const metadata = await getConfiguredChannel(socket);
      ...
      await socket.sendMessage(remoteJid, {
        newsletterFollowerInviteMessage: {
          ...
          caption: "Siga nosso canal no WhatsApp.",
        },
      });
    } catch {
      // O cartão é complementar e nunca deve impedir a resposta principal.
    }
  };
  ...
  const sendSuccessReact = async () => {
    return await sendReact(COMMAND_REACTION_EMOJI);
  };
