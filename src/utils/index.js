...
export function extractDataFromMessage(webMessage) {
  ...
  const userPhone = (webMessage?.key?.participantAlt || webMessage?.key?.participant)
...
export function getContent(webMessage, context) {
  const message =
    webMessage?.message?.ephemeralMessage?.message || webMessage?.message;
  ...
  return (
    ...
    message?.viewOnceMessage?.message?.[`${context}Message`] ||
    webMessage?.message?.extendedTextMessage?.contextInfo?.quotedMessage
      ?.viewOnceMessage?.message?.[`${context}Message`] ||
    message?.viewOnceMessageV2?.message?.[`${context}Message`] ||
    webMessage?.message?.extendedTextMessage?.contextInfo?.quotedMessage
      ?.viewOnceMessageV2?.message?.[`${context}Message`] ||
    message?.viewOnceMessageV2Extension?.message?.[`${context}Message`]
  );
}
...
let commandImportsCache = null;
let commandLookupCache = null;
let commandImportsPromise = null;

export async function findCommandImport(commandName) {
  await readCommandImports();
  return commandLookupCache?.get(formatCommand(commandName)) || {
    type: "",
    command: null,
  };
}

export async function readCommandImports({ force = false } = {}) {
  if (!force && commandImportsCache) {
    return commandImportsCache;
  }

  if (!force && commandImportsPromise) {
    return commandImportsPromise;
  }

  commandImportsPromise = (async () => {
  ...
  await Promise.all(
    ...
    commandImportsCache = commandImports;
    commandLookupCache = new Map();

    for (const [type, commands] of Object.entries(commandImportsCache)) {
      for (const command of commands) {
        if (!Array.isArray(command?.commands)) {
          errorLog(
            `Comando inválido na categoria "${type}": a propriedade "commands" precisa ser um array.`,
          );
          continue;
        }

        for (const name of command.commands) {
          const normalized = formatCommand(name);
          if (normalized && !commandLookupCache.has(normalized)) {
            commandLookupCache.set(normalized, { type, command });
          }
        }
      }
    }
  ...
  try {
    return await commandImportsPromise;
  } finally {
    commandImportsPromise = null;
  }
}

export async function refreshCommandImports() {
  commandImportsCache = null;
  commandLookupCache = null;
  return readCommandImports({ force: true });
}

export const onlyNumbers = (text) => String(text || "").replace(/[^0-9]/g, "");

export function normalizePairingPhone(text) {
  let phone = onlyNumbers(text);

  // Aceita celular brasileiro sem o nono dígito: 55 + DDD + 8 dígitos.
...
export function getRandomName(extension) {
  const fileName = `PAULOZINXZ_BOT_temp_${getRandomNumber(0, 999999)}`;
